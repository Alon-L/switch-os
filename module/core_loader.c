...
static err_t set_memory_exec(void* start, size_t size) {
  ...
  for (; (uintptr_t)addr < (uintptr_t)start + size; addr += PAGE_SIZE) {
    ...
    CHECK_TRACE(addr_pte != NULL, "Failed to find the pte of address: %lx", (uintptr_t)addr);
 ...
 */
static err_t load_exec_phys_memory(uintptr_t phys_addr, void* buf, size_t size, void** virt_addr_out) {
  err_t err = SUCCESS;
  void* virt_addr = NULL;
...
err_t load_core(struct core_header** core_header_out) {
  ...
  // active when I run....
  CHECK_RETHROW(load_exec_phys_memory(CORE_PHYS_ADDR, CORE_START, CORE_SIZE, &core_addr));
  CHECK_RETHROW(load_exec_phys_memory(CORE_RM_PHYS_ADDR, CORE_RM_START, CORE_RM_SIZE, NULL));
  CHECK_RETHROW(load_exec_phys_memory(CORE_PM_PHYS_ADDR, CORE_PM_START, CORE_PM_SIZE, NULL));
