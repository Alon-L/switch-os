...
#define BUILD_MMIO_WRITE(name, type, bwl, barrier)                                          \
  static inline __attribute__((always_inline)) void name(volatile void* addr, type value) { \
    __asm__ volatile("mov" bwl " %1, %0" : : "m"(*(volatile type*)addr), "r"(value)barrier);    \
  }
